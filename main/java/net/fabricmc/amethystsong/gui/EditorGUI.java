...
public class EditorGUI extends LightweightGuiDescription {
    ...
    public EditorGUI() {
        ...
        newSongButton.setOnClick(() -> {
            SongManager.addSong(new Song());
            int numSongs = SongManager.getNumSongs();
            SongButton songButton = new SongButton(Text.literal("Song " + numSongs),numSongs);
        ...
        for (int currentSong = 1; currentSong <= SongManager.getNumSongs(); currentSong++) {
            SongButton existingSongButton = new SongButton(Text.literal("Song " + currentSong),currentSong);
